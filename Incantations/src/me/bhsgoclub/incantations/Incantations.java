...
public class Incantations extends JavaPlugin {
	...
	private final IncantationsPlayerListener playerListener = new IncantationsPlayerListener(
			this);
	private final IncantationsBlockListener blockListener = new IncantationsBlockListener(
			this);
	private final IncantationsEntityListener entityListener = new IncantationsEntityListener(
			this);
 ...
 /*   {
	...
	public static YamlConfiguration loadConfiguration() {
		...
		log.info("[Incantations]" + " *Tarantallegra* made you dance :P"); // Because
																			// we'd
																			// rather
																			// have
																			// funny
...
*/
	/*
	 * private void setupPermissions() { if (permissions != null) return;
	 * 
	 ...
	 * usePermissions = true; log.info("[Incantations] Using " +
	 * ((Permissions)permissionsPlugin).getDescription().getFullName()); } NO
...
/*
	private void parseConfigs() {
		...
		// Read or create config file
		getDataFolder().mkdir();
		File cfgFile = new File(getDataFolder().getPath() + File.separator
				+ "config.yml");
		try {
			isNew = cfgFile.createNewFile();
		} catch (IOException e) {
			e.printStackTrace();
			log.info("[Incantations] Error occured while creating config file: "
					+ e.getMessage());
		}
		...
		File spellFile = new File(getDataFolder().getPath() + File.separator
				+ "spells.yml");
		try {
			isNew = spellFile.createNewFile();
		} catch (IOException e) {
			e.printStackTrace();
			log.info("[Incantations] Error occured while creating spell file: "
					+ e.getMessage());
		}
		...
		if (isNew) {
			...
			spells.load();
*/
		// Parse spell nodes for easy lookup <<<<AS OF 11/12/02 Temp
		// Disabled>>>>
		/*
		 * Map<String, ConfigurationNode> spellNodes =
		 * spells.getNodes("Spells"); for (Map.Entry<String, ConfigurationNode>
		 * entry : spellNodes.entrySet()) {
		 * spellLookup.put(entry.getValue().getString("Name"), entry.getKey());
		 * }
		 */
		// Read or create users file
	/*	File usersFile = new File(getDataFolder().getPath() + File.separator
				+ "users.yml");
		try {
			isNew = spellFile.createNewFile();
		} catch (IOException e) {
			e.printStackTrace();
			log.info("[Incantations] Error occured while creating users file: "
					+ e.getMessage());
		}
